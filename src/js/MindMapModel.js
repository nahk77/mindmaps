...
mindmaps.MindMapModel = function(eventBus, commandRegistry, undoController) {
  ...
  /**
   * Saves a document to a storage server and publishes DOCUMENT_SAVED event on success.
   *
   * @returns {Boolean} whether the save was successful.
   */
  this.saveToStorageServer = function(callbacks) {
    var doc = this.document.prepareSave();
    mindmaps.ServerStorage.saveDocument(doc, {
      start: function() {
        callbacks.start();
      },
      success: function() {
